...
class DataCollatorMoonshineSeq2SeqWithPadding:
    ...
    def __call__(self, features):
        ...
        label_tensors = [
            torch.tensor(labels, dtype=torch.long) for labels in label_features
        ]
...
def main():
    ...
    if args.test_mode:
        ...
        dataset_dict["train"] = dataset_dict["train"].select(
            range(min(500, len(dataset_dict["train"])))
        )
        dataset_dict["test"] = dataset_dict["test"].select(
            range(min(500, len(dataset_dict["test"])))
        )
    ...
    dataset_dict["train"] = data_loader.filter_by_duration(
        dataset_dict["train"], max_duration=max_dur, min_duration=min_dur
    )
    dataset_dict["test"] = data_loader.filter_by_duration(
        dataset_dict["test"], max_duration=max_dur, min_duration=min_dur
    )
    ...
    if config["curriculum"]["enabled"]:
        ...
        duration_col = (
            "duration"
            if "duration" in dataset_dict["train"].column_names
            else "audio_duration"
        )
    ...
    if len(dataset_dict["train"]) < 10:
        ...
        if not args.test_mode:
            ...
            print(
                "[WARNING] Test mode: Skipping confirmation prompt. Using larger sample in test mode."
            )
    ...
    dataset_dict["train"] = data_loader.prepare_dataset(
        dataset_dict["train"], processor
    )
    dataset_dict["test"] = data_loader.prepare_dataset(dataset_dict["test"], processor)
    ...
    def compute_metrics(pred):
        ...
        print("\n" + "=" * 80)
        print(
            f"EVALUATION EXAMPLES (Phase {args.phase if config['curriculum']['enabled'] else 'Full'}):"
        )
        print("=" * 80)
    ...
    training_args = Seq2SeqTrainingArguments(
        ...
        # Optimization
        optim=train_config.get(
            "optim", "adamw_torch"
        ),  # Default to standard AdamW if not specified
        learning_rate=learning_rate,
        lr_scheduler_type=train_config.get(
            "lr_scheduler_type", "linear"
        ),  # Default to linear
        warmup_steps=train_config.get(
    ...
    try:
        ...
        if "should be the same" in str(e):
            ...
            print(
                f"Your model was saved successfully to: {training_args.output_dir}/final"
            )
            print(f"\nYou can evaluate it separately with:")
            print(
                f"  python scripts/evaluate.py --model {training_args.output_dir}/final --dataset {config['dataset']['name']} --split test"
            )
            results = None
        else:
            raise
        ...
        if config["curriculum"]["enabled"]:
            ...
            if actual_wer < phase.target_wer:
                ...
                if args.phase < len(curriculum.phases):
                    ...
                    print(
                        f"   Run: python train.py --config {args.config} "
